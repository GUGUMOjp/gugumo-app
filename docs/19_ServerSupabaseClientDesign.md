...
初版作成時点では、SQL実行、Supabase変更、RLS有効化、実装変更、package変更、env変更は行っていない。

2026-07-12追記:

- Browser clientはSupabase Auth sessionをchunked cookie-backed storageへ保存する。
- Server ActionsはSupabase access tokenを引数で受け取らない。
- Server側は `src/server/core/supabaseServerClient.ts` でrequest-scoped Supabase clientを作成し、chunked cookie-backed sessionを使って `supabase.auth.getUser()` とRLS対象Repositoryを実行する。
- Service Role、global mutable session、singleton `setSession()` は使用しない。
...
- 初版時点ではBrowser用anon clientとServer側DBアクセスclientが分離されていなかった
- `NEXT_PUBLIC_` envだけを使っているため、client bundleから参照されてもよいanon key前提になっている
- 初版時点ではServer Action / Repositoryが、Server requestのCookie sessionを明示的に扱う構成になっていなかった
